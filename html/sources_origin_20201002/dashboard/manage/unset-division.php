...
<?php foreach ($resProject as $key => $val) { ?>
			...
			<td>
				<select name="division_id[<?php echo h($val['project_id']) ?>]">
					<option value=""></option>
<?php foreach ($resDivision as $key2 => $val2) { ?>
					<option value="<?php echo h($val2['division_id']) ?>"><?php echo h($val2['name']) ?></option>
<?php } ?>
				</select>
				<br>
				<input type="text" name="division_name[<?php echo h($val['project_id']) ?>]" value="">
			</td>
			</tr>
<?php } ?>
			</table>
